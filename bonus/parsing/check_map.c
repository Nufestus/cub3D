...
/*   Updated: 2025/11/25 11:21:34 by aammisse         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int check_top_bot(char **map)
{
	...
	while (map[0][i])
	{
		if (map[0][i] == '0' || strchr("NSEWD", map[0][i]))
			return (1);
		i++;
	}
	i = 0;
...
int check_edges(char **map)
{
	...
	while (map[i])
	{
		...
		|| strchr("NSEWD", map[i][0]) || strchr("NSEWD", map[i][ft_strlen(map[i]) - 1]))
			return (1);
		i++;
	}
	return (0);
...
int check_middle(char **map, t_cube *data)
{
	...
	while (map[i])
	{
		...
		while (map[i][j])
		{
			if ((map[i][j] == '0' || strchr("NSEWD", map[i][j])) && ((map[i][j + 1] && map[i][j + 1] == ' ') || (j > 0 && map[i][j - 1] && map[i][j - 1] == ' ')))
				return (1);
			else if ((map[i][j] == '0' || strchr("NSEWD", map[i][j])) && (((lengthcalc(j, map[i + 1])) || (i > 0 && lengthcalc(j, map[i - 1]))) || (i > 0 && map[i - 1][j] == ' ') || (map[i + 1] && map[i + 1][j] == ' ')))
				return (1);
